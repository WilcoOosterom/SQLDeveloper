---------------------------
--New VIEW
--MD_BUP_AKO_FILESET_V
---------------------------
CREATE OR REPLACE FORCE VIEW "RPT_VIEW_USER_RIS"."MD_BUP_AKO_FILESET_V" 
 ...
 *
 * Goal: Check on completenes for AKOFiles loaded
 *
 *----------------------------------------------------------------------------------------------------------------------*/
              MDD.BUSINESS_PARTNER
,             'AKO1xxx'||DAT.ID||'.dat'                        FILE_NAME_MASK
...
AND    MDD.BUSINESS_PARTNER = 'AKO'
AND    MDD.CURR_ID = 'Y'
LEFT OUTER JOIN
       (SELECT DISTINCT TO_NUMBER(SUBSTR(FILE_NAME,8,8)) FILE_ID FROM MD_BUP_AKO_FILE)                 FIL --compare with received files
ON     FIL.FILE_ID                                                 =  DAT.ID
LEFT OUTER JOIN MD_BUP_MISSING_FILES                                          MIF --filter out known missing files
ON     MIF.FILE_ID = DAT.ID
AND    MIF.BUSINESS_PARTNER = 'AKO'
AND    MIF.CURR_ID = 'Y'
ORDER BY DAT.ID;
  COMMENT ON TABLE "RPT_VIEW_USER_RIS"."MD_BUP_AKO_FILESET_V" IS 'Version 2019-41';
