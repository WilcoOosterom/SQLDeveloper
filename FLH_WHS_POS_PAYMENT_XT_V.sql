---------------------------
--New VIEW
--FLH_WHS_POS_PAYMENT_XT_V
---------------------------
CREATE OR REPLACE FORCE VIEW "RPT_VIEW_USER_RIS"."FLH_WHS_POS_PAYMENT_XT_V" 
 ...
 *----------------------------------------------------------------------------------------------------------------------*/
      ...
      SUBSTR(x.TEXT_LINE,INSTR (x.TEXT_LINE,';',1,1),INSTR (x.TEXT_LINE,';',-1,1)-INSTR (x.TEXT_LINE,';',1,1)+1)                  MARKETING_ATTR_KEY,
       ...
       wbx_parm.get_num ('RIS3WHSAUD')                                                                                               AUD_ID
FROM   RIS_STG.FLH_WHS_POS_REC x
WHERE  x.RECORD_TYPE = 'D'
AND    CASE x.RECORD_TYPE WHEN 'D' 
                          THEN SUBSTR (x.TEXT_LINE,5,1) END ='B'
AND    rownum <= ris3_test_flow('PF_RIS3_FILES_WHS');
